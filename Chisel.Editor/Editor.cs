...
using Chisel.Editor.Menu;
using Chisel.Editor.Settings;
using Chisel.Editor.UI;
using Chisel.Editor.UI.Sidebar;
...
using Chisel.Providers.Model;
using Chisel.Providers.Texture;
using Chisel.QuickForms;
using Chisel.Settings;
using Chisel.Settings.Models;
using Chisel.UI;
using Hotkeys = Chisel.Editor.UI.Hotkeys;
using LayoutSettings = Chisel.Editor.UI.Layout.LayoutSettings;
...
namespace Chisel.Editor
{
    public partial class Editor : HotkeyForm, IMediatorListener
    {
        ...
        private void EditorLoad(object sender, EventArgs e)
        {
            ...
            {
                var tl = tool;
                var hotkey = Chisel.Settings.Hotkeys.GetHotkeyForMessage(HotkeysMediator.SwitchTool, tool.GetHotkeyToolType());
            ...
            WadProvider.ReplaceTransparentPixels = !Chisel.Settings.View.DisableWadTransparency && !Chisel.Settings.View.GloballyDisableTransparency;
            TextureHelper.EnableTransparency = !Chisel.Settings.View.GloballyDisableTransparency;
            TextureHelper.DisableTextureFiltering = Chisel.Settings.View.DisableTextureFiltering;
            TextureHelper.ForceNonPowerOfTwoResize = Chisel.Settings.View.ForcePowerOfTwoTextureResizing;

            Subscribe();

            Mediator.MediatorException += (mthd, ex) => Logging.Logger.ShowException(ex.Exception, "Mediator Error: " + ex.Message);

            if (Chisel.Settings.View.LoadSession)
            {
        ...
        private void DoUpdateCheck(bool notify)
        {
            ...
            try
            {
                ...
                var result = GetUpdateCheckResult(ChiselWebsiteUpdateSource);
                if (result == null) return;
                if (String.Equals(result.Version, version, StringComparison.InvariantCultureIgnoreCase))
                {
                    if (notify)
                    {
                        NotifyUpdateError("This version of Chisel is currently up-to-date.", "No Updates Found");
                    }
                    return;
                }
        ...
        {
            foreach (var vp in ViewportManager.Viewports.OfType<Chisel.UI.Viewport3D>())
            {
                vp.Camera.FOV = Chisel.Settings.View.CameraFOV;
                vp.Camera.ClipDistance = Chisel.Settings.View.BackClippingPane;
            }
        ...
        {
            if (DockBottom.Hidden && Chisel.Settings.View.CompileOpenOutput) DockBottom.Hidden = false;
        }
        ...
        private void UpdateTitle()
        {
            if (DocumentManager.CurrentDocument != null)
            {
                ...
                Text = "Chisel - " + (String.IsNullOrWhiteSpace(doc.MapFile) ? "Untitled" : System.IO.Path.GetFileName(doc.MapFile));
            }
            else
            {
                Text = "Chisel";
