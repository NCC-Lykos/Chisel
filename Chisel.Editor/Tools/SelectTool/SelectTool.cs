...
namespace Chisel.Editor.Tools.SelectTool
{
    /// <summary>
    ...
    class SelectTool : BaseBoxTool
    {
        ...
        public SelectTool()
        {
            ...
            {
                Chisel.Settings.Select.Show3DSelectionWidgets = show;
        ...
        {
            get { return Color.FromArgb(Chisel.Settings.View.SelectionBoxBackgroundOpacity, Color.Gray); }
        }
        ...
        public override void MouseDoubleClick(ViewportBase viewport, ViewportEvent e)
        {
            ...
            if (Chisel.Settings.Select.DoubleClick3DAction == DoubleClick3DAction.Nothing) return;
            if (viewport is Viewport3D && !Document.Selection.IsEmpty())
            {
                if (Chisel.Settings.Select.DoubleClick3DAction == DoubleClick3DAction.ObjectProperties && !ObjectPropertiesDialog.IsShowing)
                {
                    Mediator.Publish(HotkeysMediator.ObjectProperties);
                }
                else if (Chisel.Settings.Select.DoubleClick3DAction == DoubleClick3DAction.TextureTool)
                {
        ...
        protected override void MouseDown3D(Viewport3D viewport, ViewportEvent e)
        {
            ...
            if (Chisel.Settings.View.Camera3DPanRequiresMouseClick && KeyboardState.IsKeyDown(Keys.Space)) return;
        ...
        public override void BoxDrawnConfirm(ViewportBase viewport)
        {
            ...
            if (GetSelectionBox(out boundingbox))
            {
                ...
                if (Chisel.Settings.Select.BoxSelectByCenterHandlesOnly) selector = x => Document.Map.WorldSpawn.GetAllNodesWithCentersContainedWithin(x);
