...
namespace Chisel.Editor.Tools.SelectTool.TransformationTools
{
    ...
    class MoveTool : TransformationTool
    {
        ...
        public override Matrix4? GetTransformationMatrix(Viewport2D viewport, ViewportEvent e, BaseBoxTool.BoxState state, Document doc, IEnumerable<Widget> activeWidgets)
        {
            ...
            if (state.Handle == BaseBoxTool.ResizeHandle.Center)
            {
                ...
                moveMatrix = Matrix4.CreateTranslation((float)movement.X, (float)movement.Y, (float)movement.Z);
            }
            else
            {
                var move = (state.PreTransformBoxStart - state.BoxStart) +
                             (state.BoxEnd - state.PreTransformBoxEnd);
                move = move.ComponentDivide(state.PreTransformBoxEnd - state.PreTransformBoxStart);
                move += new Coordinate(1, 1, 1);
                ...
                //Only allow Translation
                //var scale = Matrix4.Mult(trans, Matrix4.Scale((float)resize.X, (float)resize.Y, (float)resize.Z));
                var scale = trans;
                moveMatrix = Matrix4.Mult(scale, Matrix4.Invert(trans));
            }
            return moveMatrix;
        }

        private Coordinate GetMoveOrigin(Viewport2D viewport, BaseBoxTool.BoxState state, Document document)
        {
        ...
        private Coordinate GetMoveDistance(Viewport2D viewport, ViewportEvent e, BaseBoxTool.BoxState state, Document document)
        {
            var origin = GetMoveOrigin(viewport, state, document);
        ...
        private Tuple<Coordinate, Coordinate> GetBoxCoordinatesForSelectionResize(Viewport2D viewport, ViewportEvent e, BaseBoxTool.BoxState state, Document document)
        {
            ...
            switch (state.Handle)
            {
                ...
                case BaseBoxTool.ResizeHandle.Center:
                    ...
                    var distance = GetMoveDistance(viewport, e, state, document);
