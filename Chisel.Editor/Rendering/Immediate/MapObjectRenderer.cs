...
namespace Chisel.Editor.Rendering.Immediate
{
    public static class MapObjectRenderer
    {
        ...
        {
            return (float)face.Texture.Opacity;
        }
        ...
        public static void DrawWireframe(IEnumerable<Face> faces, bool overrideColor, bool drawVertices)
        {
            ...
            if (!drawVertices || !Chisel.Settings.View.Draw2DVertices) return;

            GL.PointSize(Chisel.Settings.View.VertexPointSize);
            GL.Begin(PrimitiveType.Points);
            GL.Color4(Chisel.Settings.View.VertexOverrideColour);
            foreach (var f in faces)
            {
                if (!Chisel.Settings.View.OverrideVertexColour) GL.Color4(f.Colour);
                foreach (var line in f.GetLines())
                {
                    GLCoordinate(line.Start);
                    GLCoordinate(line.End);
