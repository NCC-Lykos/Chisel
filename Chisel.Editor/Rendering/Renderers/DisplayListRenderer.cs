...
using Chisel.Editor.Rendering.Immediate;
using Chisel.Extensions;
using Chisel.Settings;
using Chisel.UI;
using Matrix = Chisel.DataStructures.Geometric.Matrix;
using Quaternion = Chisel.DataStructures.Geometric.Quaternion;
...
namespace Chisel.Editor.Rendering.Renderers
{
    public class DisplayListRenderer : IRenderer
    {
        ...
        private void UpdateCache()
        {
            ...
            var unselected = cache.Where(x => !x.IsSelected && (x.Parent == null || !x.Parent.IsSelected) && (float)x.Texture.Opacity > 0.1).ToList();
            var selected = cache.Where(x => (x.IsSelected || (x.Parent != null && x.Parent.IsSelected)) && (float)x.Texture.Opacity > 0.1).ToList();
            ...
            _transparentFaces.AddRange(selected.Where(x => (float)x.Texture.Opacity < 0.9 || (x.Texture.Texture != null && x.Texture.Texture.HasTransparency())));
            _transparentFaces.AddRange(unselected.Where(x => (float)x.Texture.Opacity < 0.9 || (x.Texture.Texture != null && x.Texture.Texture.HasTransparency())));
