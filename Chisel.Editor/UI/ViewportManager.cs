...
namespace Chisel.Editor.UI
{
    public static class ViewportManager
    {
        ...
        public static void RefreshClearColour()
        {
            ...
            {
                vp.MakeCurrent();
                GL.ClearColor(vp is Viewport3D ? Chisel.Settings.View.ViewportBackground : Chisel.Settings.Grid.Background);
            }
        }
        ...
        private static Viewport3D Create3D(Viewport3D.ViewType type)
        {
            var viewport = new Viewport3D(type)
            {
                ...
                Camera =
                {
                    ...
                    FOV = Chisel.Settings.View.CameraFOV,
                    ClipDistance = Chisel.Settings.View.BackClippingPane
                },
                VSync = false
            };
            viewport.MakeCurrent();
            GraphicsHelper.InitGL3D();
            GL.ClearColor(Chisel.Settings.View.ViewportBackground);
        ...
        private static Viewport2D Create2D(Viewport2D.ViewDirection direction)
        {
            ...
            GL.ClearColor(Chisel.Settings.Grid.Background);
            viewport.Listeners.Add(new ViewportLabelListener(viewport));
